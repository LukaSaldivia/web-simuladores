...
class CapModel extends Model{
  ...
  function getCapitulosBySeason($idSeason){
    $query = $this->db->prepare('SELECT * FROM capitulos LEFT JOIN temporadas ON capitulos.temporada = temporadas.idtemp WHERE temporada = ?');
    $query->execute([$idSeason]);
